...
public class GPSLoggerServiceManager {
	...
	/**
	 * Means by which an Activity lifecycle aware object hints about binding and
	 * unbinding
	 * 
	 * @param onServiceConnected
	 *            Run on main thread after the service is bound
	 * @param activity
	 */
	public void startup(Context context, final Runnable onServiceConnected, final Activity activity, final boolean start) {
		...
		synchronized (mStartLock) {
			if (!mBound) {
				...
				context.bindService(new Intent(Constants.SERVICENAME), this.mServiceConnection,
